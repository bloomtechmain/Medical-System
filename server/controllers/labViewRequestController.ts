  ...
  try {
    ...
    if (existing.length) {
      if (existing[0].status === 'pending') {
        res.status(409).json({ message: 'A request is already pending patient approval.' }); return;
      }
...
async function notifyPatient(labReq: Record<string, any>, doctorId: number, requestId: number): Promise<void> {
...
export { createRequest, respond, getAll, serveFile };
