...
const ACCESS_TYPES = [
  ...
  { key: 'medical_history',  label: 'Medical History',           desc: 'Consultation records, diagnoses & treatments',        Icon: ClipboardList, grad: 'from-teal-500 to-emerald-600',  light: 'bg-teal-50',   accent: 'text-teal-600'   },
  ...
  { key: 'contact_info',     label: 'Contact Information',       desc: 'Personal contact, address & insurance details',       Icon: Phone,         grad: 'from-rose-500 to-pink-600',     light: 'bg-rose-50',   accent: 'text-rose-600'   },
];
...
function LabReportsData({ reports }: { reports: any[] | null | undefined }) {
  if (!reports?.length) return <p className="text-sm text-gray-400 text-center py-4">No lab reports found.</p>;
...
function MedicalHistoryData({ consultations }: { consultations: any[] | null | undefined }) {
  ...
  if (!consultations?.length) return <p className="text-sm text-gray-400 text-center py-4">No consultation history found.</p>;
  return (
    <div className="space-y-3">
...
function PersonalReportsData({ reports }: { reports: any[] | null | undefined }) {
  if (!reports?.length) return <p className="text-sm text-gray-400 text-center py-4">No personal reports uploaded.</p>;
  return (
    <div className="space-y-2.5">
      {reports.map((r: any) => (
