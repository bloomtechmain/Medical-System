...
function Stat({ icon, label, value, color }) {
  const bg = { teal:'bg-teal-50 border-teal-100', blue:'bg-blue-50 border-blue-100', purple:'bg-purple-50 border-purple-100', orange:'bg-orange-50 border-orange-100' }[color];
  return (
...
// ── Timeline entry ──────────────────────────────────────────────
function TimelineEntry({ c, index }) {
  const [expanded, setExpanded] = useState(index === 0);

  const STATUS_COLOR = { active:'bg-yellow-100 text-yellow-700', dispensed:'bg-green-100 text-green-700', completed:'bg-gray-100 text-gray-500' };
              ...
              {c.medicines?.length > 0 && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {c.medicines.map((m) => <MedPill key={m.id} m={m} />)}
                  </div>
                </div>
              )}
...
export default function PatientHistory() {
  ...
  const sicknessLog = consultations
    ...
    .map(c => ({ date: c.visit_date, diagnosis: c.diagnosis, symptoms: c.sick_description, doctor: c.doctor_display_name }));

  // Unique medicines the patient has received
  const allMedNames = [...new Set(
    consultations.flatMap(c => (c.medicines || []).map(m => m.medicine_name))
  )];
